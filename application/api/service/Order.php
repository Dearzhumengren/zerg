...
{
    /**
     * @var 订单的商品列表，也就是客户端传递过来的Product参数
     */
    protected $oProducts;
    ...
    public static function makeOrderNo()
    {
        ...
        $orderSn = $yCode[intval(date('Y'))-2017].strtoupper(dechex(date('m'))).
            date('d').substr(time(),-5).substr(microtime(),2,5).sprintf('%02d',
                rand(0,99));
        return $orderSn;
    }
    ...
    {
        $userAddress = UserAddress::where('user_id','=',$this->id)
            ->find();
        if (!$userAddress){
